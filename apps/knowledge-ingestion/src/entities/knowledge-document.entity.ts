...
/**
 * Maps the W1-owned `knowledge_documents` table (schema lives in
 * apps/ai-copilot/src/migrations). knowledge-ingestion is a separate Nest app,
 * so it carries its own thin entity (columns only, no Tenant relation). The
 * `embedding`/`ts_vector` columns are intentionally NOT mapped here — this
 * service never writes them (W3 fills embeddings; ts_vector is GENERATED).
 */
